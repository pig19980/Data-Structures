...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2) {
	/* add your code here */
	ListNode *cur_node = ll1->head;
	ListNode *plus_node = ll2->head;
	ListNode *next_cur, *next_plus;
	while (cur_node != NULL && plus_node != NULL) {
		next_cur = cur_node->next;
		next_plus = plus_node->next;

		ll2->head = next_plus;
		plus_node->next = next_cur;
		cur_node->next = plus_node;
